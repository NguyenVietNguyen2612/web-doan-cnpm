...
const Select = ({
  ...
  return (
    <div className={`bg-gray-200 rounded-md overflow-hidden ${className}`}>
      ...
      {/* Items list */}
      <div className="bg-gray-200 min-h-[300px] max-h-[400px] overflow-y-auto">
        {items.length > 0 ? (
          <ul className="divide-y divide-gray-300">
            {items.map((item) => (
              <li 
                key={getItemKey(item)}
                className="px-4 py-2 hover:bg-gray-300 transition-colors cursor-pointer"
                ...
                <div className="flex items-center space-x-3">
                  {multiSelect && (
                    <input
                      ...
                      onChange={() => {}} // Change handled by li click
                      className="rounded border-gray-300 text-purple-600 focus:ring-purple-500"
                      onClick={(e) => e.stopPropagation()} // Prevent li click when checkbox is clicked
                    />
                  )}
