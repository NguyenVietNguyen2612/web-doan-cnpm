...
const LocationPreference = () => {
  ...
  const loadLocationPreferences = async () => {
    try {
      ...
      if (response.success) {
        ...
        setOtherPreference(data.otherPreference || '');
        
        // Lưu trạng thái ban đầu
        setOriginalLocation(data.location || '');
        setOriginalPreferences({...(data.preferences || {})});
        setOriginalOtherPreference(data.otherPreference || '');
      }
    } catch (error) {
  ...
  const handleLocationChange = (e) => {
    ...
    // Clear GPS error khi user nhập thủ công
    if (locationError) {
      setLocationError('');
    }
  };
  ...
  // Reverse Geocoding sử dụng OpenStreetMap Nominatim (miễn phí)
  ...
  const handleSave = async () => {
    try {
      const data = {
        ...
        preferences,
        otherPreference: otherPreference.trim()
      };

      const response = await locationPreferenceService.saveUserLocationPreferences(groupId, data);
      ...
      <MemberLayout rightButtons={rightButtons}>
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="mb-6 pb-4 border-b border-gray-100">
            <h2 className="text-lg font-semibold text-gray-800 mb-1">Chỉnh sửa vị trí và sở thích</h2>
            <p className="text-sm text-gray-600">Cập nhật thông tin để nhóm có thể lên kế hoạch phù hợp</p>
          </div>
                ...
                <div className="relative">
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className={`px-4 py-3 rounded-lg text-sm font-medium transition-all flex items-center gap-2 whitespace-nowrap ${
                        isGettingLocation 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
                  {preferenceOptions.map(option => (
                    <div key={option.id} className="flex items-center">
                      ...
                      <label 
                        htmlFor={option.id} 
                        className="ml-3 text-sm text-gray-700 cursor-pointer hover:text-purple-600 transition-colors"
                      >
                        {option.label}
                      </label>
                    </div>
