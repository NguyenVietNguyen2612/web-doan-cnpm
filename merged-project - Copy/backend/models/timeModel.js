...
class Time {
  ...
  static async getUserTimeslots(userId, groupId) {
    try {
      const [rows] = await db.query(`
        SELECT timeslot_id, user_id, group_id, start_time, end_time, google_calendar_event_id
        FROM TIMESLOTS 
  ...
  static async createTimeslot(timeslotData) {
    try {
      const { user_id, group_id, start_time, end_time, google_calendar_event_id } = timeslotData;
      
      const [result] = await db.query(`
        INSERT INTO TIMESLOTS (user_id, group_id, start_time, end_time, google_calendar_event_id)
        VALUES (?, ?, ?, ?, ?)
      `, [user_id, group_id, start_time, end_time, google_calendar_event_id]);
  ...
  static async updateTimeslot(timeslotId, timeslotData) {
    try {
      const { start_time, end_time, google_calendar_event_id } = timeslotData;
      
      const [result] = await db.query(`
        UPDATE TIMESLOTS 
        SET start_time = ?, end_time = ?, google_calendar_event_id = ?
        WHERE timeslot_id = ?
      `, [start_time, end_time, google_calendar_event_id, timeslotId]);
